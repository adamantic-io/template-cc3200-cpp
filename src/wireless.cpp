...
namespace {
    std::vector<adm::net::WirelessConnection::ConnectionUpListener>   CONN_UP_LST;
    std::vector<adm::net::WirelessConnection::ConnectionDownListener> CONN_DOWN_LST;
    std::vector<adm::net::WirelessConnection::ConnectionFailListener> CONN_FAIL_LST;
    std::vector<adm::net::WirelessConnection::IpAcquiredListener>     IP_ACQ_LST;

    std::string CUR_SSID_KEY; // hold against pointer invalidation (c_str());
    unsigned char WIRELESS_POLICY_VAL = 0;  // hold the wireless policy value
}
...
#define SL_STOP_TIMEOUT         200
    ...
    #include "logger.h"
    #define UART_PRINT adm::util::Logger::i().log
    #define DBG_PRINT  adm::util::Logger::i().log
    #define ERR_PRINT(x) adm::util::Logger::i().log("Error [%d] at line [%d] in function [%s]  \n\r",x,__LINE__,__FUNCTION__)
#else
    #include "uart_if.h"
    ...
    if(pSlWlanEvent == NULL)
    {
     ...
     switch(((SlWlanEvent_t*)pSlWlanEvent)->Event)
     {
        ...
        case SL_WLAN_DISCONNECT_EVENT:
        {
            ...
            bool user_initiated = (SL_WLAN_DISCONNECT_USER_INITIATED_DISCONNECTION == pEventData->reason_code);
            for (auto lst : CONN_DOWN_LST) lst(user_initiated);
            if (user_initiated) {
...
void SimpleLinkNetAppEventHandler(SlNetAppEvent_t *pNetAppEvent)
{
    ...
    switch(pNetAppEvent->Event)
    {
        case SL_NETAPP_IPV4_IPACQUIRED_EVENT:
        {
            ...
            std::string ip = adm::util::format("%d.%d.%d.%d",
                (pEventData->ip & 0xFF000000) >> 24,
                (pEventData->ip & 0x00FF0000) >> 16,
                (pEventData->ip & 0x0000FF00) >> 8,
                pEventData->ip & 0x000000FF);

            UART_PRINT("[NETAPP EVENT] IP '%s' acquired by the device\n\r", ip.c_str());
            for (auto lst : IP_ACQ_LST) lst(ip);
        }
        break;
        ...
        case SL_NETAPP_IPV6_IPACQUIRED_EVENT:
        {
            ...
            std::string ip = adm::util::format("%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
                (pEventData->ip[0] & 0xF0) >> 4, pEventData->ip[0] & 0x0F,
                (pEventData->ip[1] & 0xF0) >> 4, pEventData->ip[1] & 0x0F,
                (pEventData->ip[2] & 0xF0) >> 4, pEventData->ip[2] & 0x0F,
                (pEventData->ip[3] & 0xF0) >> 4, pEventData->ip[3] & 0x0F);
...
//*****************************************************************************
// adm::net::WirelessConnection - implementation of member functions
//*****************************************************************************
...
void WirelessConnection::wlan_connect(const std::string& ssid, const std::string& key, SecType sec_type) {
    //
    // Reset The state of the machine
    //
    Network_IF_ResetMCUStateMachine();
     UART_PRINT("######## Network_IF_ResetMCUStateMachine: DONE.\n\r");

    //
    // Start the driver
    //
    long retval;
    if((retval=Network_IF_InitDriver(ROLE_STA)) < 0)
    {
       UART_PRINT("Failed to start SimpleLink Device: %ld\n\r", retval);
       goto connaborted;
    }
    UART_PRINT("######## Network_IF_InitDriver: DONE.\n\r");

    CUR_SSID_KEY = key;
    SlSecParams_t secParms;
    secParms.Key = (signed char *)CUR_SSID_KEY.c_str();
    secParms.KeyLen = CUR_SSID_KEY.length();
    switch (sec_type) {
        case OPEN:     secParms.Type = SL_SEC_TYPE_OPEN; break;
        case WEP:      secParms.Type = SL_SEC_TYPE_WEP; break;
        case WPA_WPA2: secParms.Type = SL_SEC_TYPE_WPA_WPA2; break;
        default:
            UART_PRINT("Unhandled wireless security type: %d\n\r", sec_type);
            goto connaborted;
    }
    UART_PRINT("########  security type: DONE.\n\r");

    //
    // Connect to the Access Point
    //
    if((retval=Network_IF_ConnectAP((char*)ssid.c_str(), secParms)) < 0)
    {
       UART_PRINT("Connection to an AP %s failed: %ld\n\r", ssid.c_str(), retval);
       goto connaborted;
    }
    UART_PRINT("########  Network_IF_ConnectAP: DONE.\n\r");

    retval = sl_WlanProfileAdd((const signed char*)ssid.c_str(),ssid.length(),0,&secParms,0,1,0);

    UART_PRINT("########  sl_WlanProfileAdd: DONE.\n\r");

    //set AUTO policy
    retval = sl_WlanPolicySet(SL_POLICY_CONNECTION,
                      SL_CONNECTION_POLICY(1,0,0,0,0),
                      &WIRELESS_POLICY_VAL, 1 /*PolicyValLen*/);  

    UART_PRINT("########  sl_WlanPolicySet: DONE.\n\r");

    return;

connaborted:
    for (auto lst : CONN_FAIL_LST) lst();
}

void WirelessConnection::wlan_disconnect() {
    Network_IF_DisconnectFromAP();
    Network_IF_DeInitDriver();
    Network_IF_ResetMCUStateMachine();
}

bool WirelessConnection::is_connected() {
    return IS_CONNECTED(g_ulStatus);
}


bool WirelessConnection::is_ip_acquired() {
    return IS_IP_ACQUIRED(g_ulStatus);
}

void WirelessConnection::on_connection_up(ConnectionUpListener lst) {
    CONN_UP_LST.push_back(lst);
}

void WirelessConnection::on_connection_down(ConnectionDownListener lst) {
    CONN_DOWN_LST.push_back(lst);
}

void WirelessConnection::on_connection_fail(ConnectionFailListener lst) {
    CONN_FAIL_LST.push_back(lst);
}

void WirelessConnection::on_ip_acquired(IpAcquiredListener lst) {
    IP_ACQ_LST.push_back(lst);
}
}} // namespace adm::net